...
if not (smtp_host and smtp_user and smtp_pass):  
    print(f"{Fore.RED} [ERROR] SMTP credentials are missing in .env file. Exiting.{Style.RESET_ALL}")  
    exit(1)  
  
config_path = 'config.json'  
if not os.path.exists(config_path):  
    print(f"{Fore.RED}Config file '{config_path}' not found.{Style.RESET_ALL}")  
    exit(1)  
...
def create_message(email, html_body, plain_text, subject, sender_name, from_email, sender_domain):  
    ...
    x_mailer = random.choice(x_mailer_options)  
    ...
    msg.add_header('List-Id', f"{sender_name} <{random.choice(['support', 'info', 'contact', 'noreply'])}@{sender_domain}>")  
...
def main():  
    ...
    try:  
        ...
        for email in emails:  
            ...
            ranum = generate_random_number()  # Generate 7-digit random number  
              
            # Process subject with all placeholders including {ranum}  
            subject = subject_template.replace('{name}', recipient_name)\  
                .replace('{date}', today)\  
                .replace('{email}', email) \  
                .replace('{otp}', otp_code) \  
                .replace('{case_id}', case_id) \  
                .replace('{ranum}', str(ranum))  
              
            # Process HTML body with all placeholders including {ranum}  
            html_body = email_body_template \  
            ...
            print(f"{Fore.GREEN}{Style.BRIGHT} [+]FUCKED => {Fore.CYAN}{email}{Style.RESET_ALL}")  
            counter += 1  
